...
## Follow-up: the labelled message got answered as a form (2026-08-07)

The context feature above introduced a failure it did not anticipate. A deployed row read
`来源：Hacker News\n标题：网页服务器部署模式在业余规模下行不通`, which the reader rendered as one run-on
headline — the newline collapses to a space in HTML, so 来源 and 标题 appeared to be part of the title.

**Cause, not blame on the model.** `userMessage` sends a labelled multi-line form, and the likeliest
completion of a form is the same form filled in. The system prompt's counter-instruction ("需要翻译的
永远只有「标题：」后面那一行") is prose competing with the structure of the model's own input, and
structure wins. It showed up on Hacker News because its `<description>` is `<a …>Comments</a>`, which
`summaryContext` correctly drops — leaving a two-line message that reads even more like a blank to
fill than the three-line one.

Two changes, at different layers:

- **Cause:** `examplePairs` / `exampleTurns` put a fixed user/assistant demonstration between the
  system prompt and the real request — one exchange per shape `userMessage` produces (with 摘要 and
  without), each answered with a bare headline. The example user messages are *generated by calling
  `userMessage`*, so the demonstrated format cannot drift from the sent format. Both stay in the
  cacheable prefix, so the recurring cost is a longer cached prefix, not longer billed input.
- **Consequence:** `stripEchoedLabels` takes the `标题` line when the answer carries labels, drops
  echoed 来源/摘要 lines, and otherwise keeps the first non-empty line (which also trims commentary a
  model appends under the headline). It runs before `unwrapQuotes`, so `标题：“译文”` still unwraps.

Rejected alternatives, recorded because both were asked for directly:

- **Tuning `temperature`.** Nothing to gain: a title is translated exactly once and the result is
  permanent, so there is never a second sample whose divergence a lower temperature would suppress.
  It also points the wrong way — low temperature favours the literal register that this prompt exists
  to fight — and reasoning models reject `temperature != 1`, which the 400 retry would report to the
  user as "模型不可用" while dropping `thinking` for no reason.
- **`response_format: json_object`.** Its value is aligning multi-element responses, and this design
  is deliberately one title per request, so it would wrap a single string. It does not even solve the
  bug: the model can return `{"title":"来源：… 标题：…"}` and the label is still there, now behind a
  parser that turns a truncated or fence-wrapped body into total loss. Plus DeepSeek's JSON mode
  wants "json" and a format sample in the prompt, i.e. a rewrite of the byte-stable prefix, and
  support across the OpenAI-compatible endpoints a user may configure is uneven.

The general rule this settles, now in CLAUDE.md: model output is untrusted input. Instruct,
demonstrate, then validate — a demonstration moves the failure rate but never to zero, which is why
`unwrapQuotes`, `maxGrowth` and `stripEchoedLabels` all stay even though the prompt forbids what each
of them catches.

Not done, still open: rows already stored with the echoed prefix. The worker only reconsiders
`title_zh IS NULL AND pub_ts > now − 24h`, so resetting affected rows to NULL recovers only the last
day's; anything older would need the bound widened for one run, which also re-bills every settled
`''` row it sweeps.
