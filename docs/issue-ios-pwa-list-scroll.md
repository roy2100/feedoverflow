...
- Gate / remove the permanent `willChange: 'transform'` on the panels (it forces a persistent GPU layer even at rest — a possible remaining compositing trigger that attempt #3 did not remove). **Done 2026-07-25**: `willChange` is now gone from the panels entirely (`plan-drop-mobile-history.md`; it was briefly only dropped on the swipe-back render in `e0081a9`, see `issue-ios-swipe-back-scroll-stuck.md`). The panels are no longer promoted at rest at all, so **retest this issue on iOS** — if it still reproduces, the compositing theory above is dead and the `100dvh` / dynamic-viewport theory below is the one to pursue.
