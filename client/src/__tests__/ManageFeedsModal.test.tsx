...
describe('ManageFeedsModal translate toggle', () => {
  // The modal asks the server whether an LLM key is configured; without a stub
  // the fetch rejects and the toggle stays disabled, which would make every
  // assertion below vacuous.
  function stubLLMConfig(keySet: boolean) {
    vi.stubGlobal(
      'fetch',
      vi.fn().mockResolvedValue({
        ok: true,
        json: async () => ({ base_url: 'https://api.deepseek.com', model: 'm', key_set: keySet }),
      }),
    );
  }

  it('sends translate_enabled alone', async () => {
    stubLLMConfig(true);
    const { onUpdate } = renderModal();

    await waitFor(() => expect(screen.getByTitle('开启标题翻译')).toBeInTheDocument());
    fireEvent.click(screen.getByTitle('开启标题翻译'));

    await waitFor(() => expect(onUpdate).toHaveBeenCalled());
    // Exactly translate_enabled — the same independence the push toggle needs, so
    // the two opt-ins can never clear each other.
    expect(onUpdate).toHaveBeenCalledWith('1', { translate_enabled: true });
  });

  // Without a key the switch would flip a feed flag and never change a title.
  // Better to say why than to look broken.
  it('is inert and self-explaining when no API key is configured', async () => {
    stubLLMConfig(false);
    const { onUpdate } = renderModal();

    const btn = await screen.findByTitle('未配置翻译服务 · 请先在设置中填写 API Key');
    fireEvent.click(btn);

    expect(onUpdate).not.toHaveBeenCalled();
  });

  // Turning translation on must not touch push registration: they are unrelated
  // axes, and one silently subscribing the device would be a surprise.
  it('does not register the device for push', async () => {
    stubLLMConfig(true);
    renderModal();
    ...
    await waitFor(() => expect(ensureSubscribed).not.toHaveBeenCalled());
  });
});
