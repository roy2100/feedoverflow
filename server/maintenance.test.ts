...
function addArticle(id: string, feedId: string, opts: { starred?: 0 | 1; pubDate?: string; content?: string } = {}) {
  insert.run({
    id, feedId,
...
test('cleanupOrphans — 删除非收藏孤儿，保留收藏孤儿和有源文章', () => {
  reset();
  ...
  const ids = (db.prepare('SELECT article_id FROM article_states').all() as Array<{ article_id: string }>).map(r => r.article_id).sort();
  assert.deepEqual(ids, ['a-live', 'a-orphan-starred']);
});

// ── enforceSizeCap ────────────────────────────────────────────────────────────

test('enforceSizeCap — 低于上限时不删除', () => {
  reset();
...
test('enforceSizeCap — 超出上限时按时间从旧到新删除，保留收藏文章', () => {
  reset();
  // Oldest is starred to prove starred is never deleted even when it is the oldest.
  addArticle('star-oldest', 'f', { starred: 1, pubDate: '2026-01-01T00:00:00Z', content: 'x'.repeat(50000) });
  ...
  const ids = (db.prepare('SELECT article_id FROM article_states').all() as Array<{ article_id: string }>).map(r => r.article_id).sort();
  assert.deepEqual(ids, ['new', 'star-oldest'], 'oldest non-starred deleted; newest + starred kept');
});
