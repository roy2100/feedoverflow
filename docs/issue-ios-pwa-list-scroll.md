...
- Gate / remove the permanent `willChange: 'transform'` on the panels (it forces a persistent GPU layer even at rest — a possible remaining compositing trigger that attempt #3 did not remove). **Partly done 2026-07-25** (`e0081a9`): `willChange` is now dropped on the iOS swipe-back (`instant`) render — see `issue-ios-swipe-back-scroll-stuck.md`, which fixed a *different* symptom (scroll dead for one touch after swipe-back) on these same panels. It only removes the layer on the `instant` path, not at rest after a forward tap, so this mis-paint may still reproduce; worth **retesting this issue on iOS** now that the panels de-promote on back.
